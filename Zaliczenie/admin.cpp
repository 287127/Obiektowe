    ...
    , AdminUi(new Ui::Admin)
{
    ...
    AdminUi->UsrMenu->hide();
    AdminUi->SubjMenu->hide();
    AdminUi->ClassMenu->hide();
    AdminUi->SchdMenu->hide();
...
int db_create_rest(QSqlDatabase db, Ui::Admin *AdminUi)
{
    ...
    success = query.exec(
        "INSERT OR IGNORE INTO Users (username, passwrd, role_id) VALUES "
        "('jkowalski', '123', 2),"
        "('anowak', '123', 3),"
        "('mpiatek', '123', 3),"
        "('pkos', '123', 3),"
    ...
    success = query.exec(
        "INSERT OR IGNORE INTO Teachers (name, surname, role_id, user_id) VALUES "
        "('Jan', 'Kowalski', 2, 2),"
        "('Joanna', 'Kaczmarek', 2, 6);");
    ...
    success = query.exec(
        "INSERT OR IGNORE INTO Students (name, surname, role_id, user_id, class_id) VALUES "
        "('Anna', 'Nowak', 3, 3, 1),"
        "('Marcin', 'Piątek', 3, 4, 1),"
        "('Paweł', 'Kosiński', 3, 5, 2);");
    ...
    success = query.exec(
        "INSERT OR IGNORE INTO TeachSubjs (teach_id, subj_id) VALUES "
            "(1, 1), (1, 4), (2, 2), (2, 3);");
    ...
    success = query.exec(
        "INSERT OR IGNORE INTO Schedules (dotw, hour, subj_id, teach_id, class_id) VALUES "
        "('poniedzialek', '08:00', 1, 1, 1),"
        "('poniedzialek', '09:00', 2, 2, 1),"
        "('wtorek', '08:00', 1, 1, 2),"
        "('wtorek', '09:00', 3, 2, 1),"
        "('sroda', '10:00', 4, 1, 3);");
    ...
    success = query.exec(
        "INSERT OR IGNORE INTO GradeTypes (name) VALUES "
        "('sprawdzian'), ('kartkówka'), ('odpowiedź'), ('zadanie domowe');");
    ...
    success = query.exec(
        "INSERT OR IGNORE INTO Grades (val, date, stud_id, teach_id, subj_id) VALUES "
        "(5.0, '2025-06-10', 1, 1, 1),"
        "(4.5, '2025-06-11', 1, 2, 2),"
        "(3.0, '2025-06-12', 2, 1, 1),"
        "(2.5, '2025-06-12', 3, 1, 1),"
        "(4.0, '2025-06-13', 2, 2, 3),"
        "(5.5, '2025-06-13', 3, 2, 2);");

    if (!success) {
        return 5;
    }

    success = query.exec("INSERT OR IGNORE INTO Attendance (state, stud_id, less_id) VALUES "
        "('obecny', 1, 1),"
        "('spóźniony', 2, 1),"
        "('nieobecny', 3, 1),"
        "('usprawiedliwiony', 2, 2),"
        "('obecny', 1, 4),"
        "('nieobecny', 1, 5),"
        "('obecny', 2, 5);"
        );
...
void Admin::on_AdminUsrBtn_toggled(bool checked)
{
    if(checked){
        int baza_created = ::db_create_rest(db, AdminUi);
        if(baza_created != 5){
            AdminUi->UsrMenu->show();
...
void Admin::on_AdminClassBtn_toggled(bool checked)
{
    int baza_created = ::db_create_rest(db,AdminUi);
    if(checked){
        if(baza_created != 5){
            AdminUi->ClassMenu->show();
...
void Admin::on_AdminSubjBtn_toggled(bool checked)
{
    int baza_created = ::db_create_rest(db,AdminUi);
    if(checked){
        if(baza_created != 5){
...
void Admin::on_AdminSchdBtn_toggled(bool checked)
{
    int baza_created = ::db_create_rest(db,AdminUi);
    if(checked){
        if(baza_created != 5){
            AdminUi->SchdMenu->show();
            ::db_class_list(db, AdminUi, ClassModel);
            AdminUi->AdminSubjBtn->setChecked(false);
            AdminUi->AdminUsrBtn->setChecked(false);
            AdminUi->AdminClassBtn->setChecked(false);
